...
const EditPostPage = () => {
    ...
    const { data: postData, isLoading: isPostLoading, isError: isPostError, error: postError } = useQuery(
        ['post', identifier],
        () => {
            console.log('Fetching post data for identifier:', identifier);
            return postActions.getPost(identifier).then((response) => response.data.data);
        },
        {
            staleTime: 0, // 5 minutes to reduce refetches
            ...
            onSuccess: (post) => {
                ...
                if (!isPostInitialized) {
                    ...
                    setMediaPreviews(normalized.map((att) => ({
                    ...
                    console.log('Post already initialized, skipping state update');
                }
            },
        }
    );
    ...
    const handleMediaRemove = (index) => {
        ...
        console.log('Видалення прев’ю:', preview.key, 'Поточні mediaPreviews:', JSON.stringify(mediaPreviews));
        ...
        console.log('Оновлено mediaPreviews:', JSON.stringify(newPreviews));
        setMedia(newMedia);
    ...
    useEffect(() => {
        console.log('Поточний стан mediaPreviews:', JSON.stringify(mediaPreviews));
        ...
        return () => {
            console.log('Очищення blob URLs при розмонтуванні:', Array.from(blobUrls.keys()));
            blobUrls.forEach((url) => URL.revokeObjectURL(url));
            setBlobUrls(new Map());
        };
    }, []); // Виконувати лише при монтуванні/розмонтуванні

    const fetchAttachmentAsFile = async (attachment) => {
        try {
            const response = await window.axios.get(`${STORAGE_URL}${attachment.url}`, {
                responseType: 'blob',
            });
            ...
            console.error('Помилка при отриманні вкладення:', attachment.url, error);
    ...
    const handleSubmit = async () => {
        ...
        // Додаємо нові файли з media
        console.log('Додаємо нові файли з media:', media.map((f) => f.name));
        media.forEach((file, index) => {
            formData.append('attachments[]', file);
            console.log(`Додано новий файл: ${file.name} (індекс: ${index})`);
        });

        // Конвертуємо існуючі вкладення (які не видалено) у файли
        const remainingAttachments = existingAttachments.filter((att) => !removedAttachmentUrls.includes(att.url));
        console.log('Залишилися вкладення для конвертації:', remainingAttachments.map((att) => att.url));
        ...
        // Логування вмісту FormData для дебагу
