...
const fetchPosts = async ({ pageParam = null }) => {
    try {
        ...
        const posts = response.data.data.map(post => ({
            ...
            userAvatar: post.user?.avatar || `https://i.pravatar.cc/150?img=${post.user?.id || 0}`,
        ...
        return {
            ...
            totalPosts: response.data.meta?.total || 0
        };
    } catch (error) {
...
const fetchTopPosts = async () => {
    try {
        const response = await postActions.getPosts({ sort: 'top', limit: 5 });
        return {
    ...
    const FiltersSidebar = () => (
        <Paper sx={{
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                <FilterList sx={{ mr: 1, color: '#9c27b0' }} />
                <Typography variant="h6" sx={{ color: '#ffffff' }}>Filters</Typography>
                {isMobile && (
                    <IconButton onClick={toggleFilters} sx={{ ml: 'auto', color: '#ffffff' }}>
                        <Close />
                    </IconButton>
                )}
            </Box>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                <Button
                    startIcon={<Whatshot sx={{ color: filter === 'hot' ? '#ff4081' : '#b0b0b0' }} />}
                    onClick={() => setFilter('hot')}
                    sx={{
                        justifyContent: 'flex-start',
                        color: filter === 'hot' ? '#ff4081' : '#e0e0e0',
                        bgcolor: filter === 'hot' ? 'rgba(255, 64, 129, 0.1)' : 'transparent',
                        '&:hover': {
                ...
                <Button
                    startIcon={<NewReleases sx={{ color: filter === 'new' ? '#ff4081' : '#b0b0b0' }} />}
                    onClick={() => setFilter('new')}
                    sx={{
                        justifyContent: 'flex-start',
                        color: filter === 'new' ? '#ff4081' : '#e0e0e0',
                        bgcolor: filter === 'new' ? 'rgba(255, 64, 129, 0.1)' : 'transparent',
                        '&:hover': {
                            bgcolor: 'rgba(255, 255, 255, 0.08)'
                        }
                    }}
                ...
                <Button
                    startIcon={<TrendingUp sx={{ color: filter === 'top' ? '#ff4081' : '#b0b0b0' }} />}
                    onClick={() => setFilter('top')}
                    sx={{
                        justifyContent: 'flex-start',
                        color: filter === 'top' ? '#ff4081' : '#e0e0e0',
                        bgcolor: filter === 'top' ? 'rgba(255, 64, 129, 0.1)' : 'transparent',
                        '&:hover': {
                            bgcolor: 'rgba(255, 255, 255, 0.08)'
                        }
                    }}
            ...
            <Typography variant="subtitle2" sx={{ mt: 3, mb: 1, color: '#b0b0b0' }}>
                Categories
            </Typography>
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                {['Technology', 'Science', 'Art', 'News', 'Sports'].map((cat) => (
                    <Chip
                        key={cat}
                        label={cat}
                        clickable
    ...
    const TopPostsSidebar = () => (
        <Paper sx={{
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                ...
                <Typography variant="h6" sx={{ color: '#ffffff' }}>Top Posts</Typography>
            </Box>
            <Divider sx={{ bgcolor: 'rgba(255, 255, 255, 0.12)', mb: 2 }} />
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                    {Array.from({ length: 3 }).map((_, i) => (
                        <Box key={`top-skeleton-${i}`} sx={{ display: 'flex' }}>
                            <Skeleton variant="rectangular" width={60} height={60} sx={{
                                borderRadius: 1,
                                mr: 2,
                                bgcolor: 'rgba(255, 255, 255, 0.1)'
                            }} />
                            <Box sx={{ flexGrow: 1 }}>
                                <Skeleton width="80%" height={20} sx={{
                                    mb: 1,
                                    bgcolor: 'rgba(255, 255, 255, 0.1)'
                                }} />
                                <Skeleton width="60%" height={16} sx={{
                                    bgcolor: 'rgba(255, 255, 255, 0.1)'
                                }} />
                            </Box>
                        </Box>
                                        ...
                                        <Paper sx={{
                                            ...
                                            <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                                                ...
                                                <Box sx={{ ml: 2, flexGrow: 1 }}>
                                                    <Skeleton width="60%" height={20} sx={{
                                                        bgcolor: 'rgba(255, 255, 255, 0.1)'
                                                    }} />
                                                    <Skeleton width="40%" height={16} sx={{
                                                        bgcolor: 'rgba(255, 255, 255, 0.1)'
                                                    }} />
                                                </Box>
                                            </Box>
                                            <Skeleton width="90%" height={24} sx={{
                                                mb: 1,
                                                bgcolor: 'rgba(255, 255, 255, 0.1)'
                                            }} />
                                            <Skeleton width="80%" height={18} sx={{
                                                mb: 2,
                                                bgcolor: 'rgba(255, 255, 255, 0.1)'
                                            }} />
                                            ...
                                            <Box sx={{ display: 'flex', mt: 2 }}>
                                                <Skeleton width={80} height={36} sx={{
                                                    mr: 1,
                                                    borderRadius: 18,
                                                    bgcolor: 'rgba(255, 255, 255, 0.1)'
                                                }} />
